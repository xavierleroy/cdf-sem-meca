...
(** La terminaison sur une valeur [v] est définie par un prédicat inductif
    [terminates x v].
    La divergence est définie par un prédicat coinductif [diverges x]. *)
...
(** La bisimulation faible.  C'est une autre présentation de l'équiterminaison,
    avec des constructeurs [bisim_left] et [bisim_right] qui permettent
    de "sauter" un constructeur [later] d'un côté mais pas de l'autre.
    Ceci facilite certaines démonstrations.  Cependant, il faut empêcher
    d'appliquer [bisim_left] ou [bisim_right] une infinité de fois,
    car cela rendrait bisimilaires [now v] et [bottom].  L'argument de type
    [nat] permet de limiter le nombre d'applications consécutives
    des règles [bisim_left] et [bisim_right].  Il est remis à une valeur
    arbitrairement élevée lorsque [bisim_both] est appliqué. *)
...
(** La sémantique d'un arbre de syntaxe abstraite (de type [mon A])
    est définie par traduction vers le calcul (de type [delay A])
    qu'il dénote. *)
...
(** Pour certaines démonstrations, on utilise des continuations, que l'on peut
    aussi voir comme des contextes, et qui se composent d'une liste de
    fonctions [A -> mon B] qui peuvent se composer. *)
...
(** La deuxième loi monadique est particulièrement difficile à démontrer.
    On utilise la bisimulation en lien avec la relation ci-dessous. *)
...
Proof.
  ...
  - destruct m.
    ...
    + rewrite ! run_Bind_Bind.
      ...
      apply eta_match_2 with (k := Kbind m0 k).
Qed.
